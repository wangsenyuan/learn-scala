...
class MTree[+T](value: T, children: List[MTree[T]]) {
  ...
  //  def internalPathLength: Int = internalPathLengthInteral(0)
  //  
  //  def internalPathLengthInteral(fromRoot: Int): Int = {
  //    val ss = children.map(_.internalPathLengthInteral(fromRoot + 1))
  //    ss.foldLeft(fromRoot)(_ + _)
  //  }
  ...
  def lispyTree: String = children match {
    case Nil => s"$value"
    case list => s"($value ${list.map(_.lispyTree).mkString(" ")})"
  }
}
