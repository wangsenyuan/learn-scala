...
import fp.scalacheck.exhaustive.Gen._
import fp.scalacheck.exhaustive.Prop._
import fp.scalacheck.exhaustive.Status._
import fp.state._
...
object Gen {
  ...
  def choose(start: Int, stopExclusive: Int): Gen[Int] = {
    val state = State(RNG.positiveInt).map(n => start + n % (stopExclusive - start))
    val stream = Stream.from(start).take(stopExclusive - start) map (Some(_))
    Gen(state, stream)
  }
